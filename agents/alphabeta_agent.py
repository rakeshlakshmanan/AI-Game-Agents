...
class AlphaBetaAgent(BaseAgent):
    ...
    def _alphabeta(self, game, depth, is_maximizing, alpha, beta) -> float:
        ...
        if is_maximizing:
            ...
            for move in valid_moves:
                ...
                score = self._alphabeta(g, depth + 1, False, alpha, beta)
                best = max(best, score)
            ...
            for move in valid_moves:
                g = game.clone()
                g.make_move(move, current_player)
                score = self._alphabeta(g, depth + 1, True, alpha, beta)
                best = min(best, score)
                beta = min(beta, best)
                if beta <= alpha:
                    break
